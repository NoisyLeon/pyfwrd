...
c-----
     ...
     1              sinpr, sinsvr, pex,svex,iwat(m),dble(d(m)))
...
c       cosp=cosh(p)  rsinp =rp *sinh(p)  
c              if(cdabs(rp) .lt.1.0e-8)
c                       sinpr = dm
c              else
c                       sinpr = sinh(p)/rp
c       cosq=cosh(q)  rsinsv=rsv*sinh(p)  
c              if(cdabs(rsv) .lt.1.0e-8)
c                       sinqr = dm
c              else
c                       sinqr = sinh(q)/rsv
c-----
